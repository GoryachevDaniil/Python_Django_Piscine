...
def titlescreen(request):
    return render(request, "moviemon/titlescreen.html",
                  {'commands': {'btn_a': 'worldmap/?key=new_game',
...
def moviedex(request):
    ...
    if movie_count == 0:
        ...
        if key < 0:
            ...
            key -= movie_count
    for i in show_list:
        idx = my_info.movie.captured[i]
        post_list.append(my_info.movie.moviedex[idx]['Poster'])
        title_list.append(my_info.movie.moviedex[idx]['Title'])
    if movie_count > 0:
        moviemon_id = my_info.movie.captured[key]

    return render(request, "moviemon/moviedex.html",
                  {'commands': {'btn_a': btn_a + moviemon_id + '?key=' + str(key),
                                'btn_b': '#',
                                'btn_start': '#',
                                'btn_select': '../worldmap/',
                                'btn_up': '#',
...
def save(request):
    ...
    if save_slot == True:
        game.GameData.save(indicator)

    sav_lst = game.GameData.get_save_list()
...
def load(request):
    ...
    if load_slot == True:
        if game.GameData.load(indicator) == True:
...
def error404(request, exception):
    return render(request, "moviemon/404.html", {'exception': exception})
